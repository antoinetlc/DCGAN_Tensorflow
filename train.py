...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(startStep+1, NUM_TRAINING_STEPS):
        
            #Load training data and reshape (use np.reshape ! not tf.reshape otherwise a new node is added to the graph at each iteration and the training gets slow after several iterations)
            xBatch, yBatch  = mnist.train.next_batch(BATCH_SIZE)
            xBatch = np.reshape(xBatch, [-1, 28, 28,1])
            
            #0 padding to get to size (32,32,1)
            images = np.zeros((BATCH_SIZE, WIDTH, HEIGHT, NUM_COLOR_CHANNEL))
            images[:,0:28,0:28,:] = xBatch
            ...
            #Note the we have to feed numpy arrays hence use eval()
            sess.run(discriminatorOptimizer, {imagesBatch:images, latentCode:generateLatentCode(BATCH_SIZE, LATENT_CODE_SIZE)})
                
            #Optimize the generator
            #Note the we have to feed numpy arrays hence use eval()
            #sess.run(generatorOptimizer, {imagesBatch:images, latentCode: generateLatentCode(BATCH_SIZE, LATENT_CODE_SIZE)})
            
            #Optimize the generator 2
            #Note the we have to feed numpy arrays hence use eval()
            _, currentDiscrLoss, currentGenLoss, currProbaReal, currProbaFake = sess.run([generatorOptimizer, discriminatorLoss, generatorLoss, probabilityReal, probabilityFake], {imagesBatch:images, latentCode: generateLatentCode(BATCH_SIZE, LATENT_CODE_SIZE)})
                        
            
            print("Generator Loss : ", currentGenLoss)
            print("Discriminator Loss  : ", currentDiscrLoss)
            #print("probability real : ", currProbaReal[0,:] )
            #print("probability fake : ", currProbaFake[0,:] )
              
            #Global step is increased
            sess.run(incrementGlobalStepOp, {imagesBatch:images, latentCode: generateLatentCode(BATCH_SIZE, LATENT_CODE_SIZE)})
            
            if i%1000 == 0:
                print("Saving Summary")
                
                #Saves summary at each epoch
                #Only save every N step otherwise takes a too much space on disk
                saver.save(sess, checkpointDir + "/model.ckpt", global_step = globalStep)
                    
                #Saves a generated result to binary
                currentGenImage = sess.run(generatedImages, {imagesBatch:images, latentCode:generateLatentCode(BATCH_SIZE, LATENT_CODE_SIZE)})
                     
                summary = sess.run(merged, {imagesBatch:images, latentCode:generateLatentCode(BATCH_SIZE, LATENT_CODE_SIZE)})
                writer.add_summary(summary, i)  # Write summary
                   
                fileName = "genImage_" + str(i) + ".png"
                    
                currentGenImage += 1.0
                currentGenImage /= 2.0
                currentGenImage *= 255.0
                imageio.imwrite(fileName, np.array(np.floor(currentGenImage[0,:,:,:]), dtype=np.uint8)) #Save the first frame too
